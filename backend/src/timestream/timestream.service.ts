...
export class TimestreamService {
  ...
  private async handleQuery(queryRequest: string) {
    ...
    try {
      const response: QueryCommandOutput = await this.queryClient.send(
        queryCommand,
      );
      return response;
    } catch (error) {
  ...
  async getDeviceData(
    ...
    if (filterDto) {
      if (filterDto.minTime) {
        queryRequest += ` AND time >= '${this.unixToDatetime(
          parseInt(filterDto.minTime),
        )}'`;
      }
      if (filterDto.maxTime) {
        queryRequest += ` AND time <= '${this.unixToDatetime(
          parseInt(filterDto.maxTime),
        )}'`;
      }
    }
